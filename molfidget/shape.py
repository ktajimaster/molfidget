...
from molfidget.config import ShapeConfig, BondTypeShapeConfig
import numpy as np

class Shape:
    def __init__(self, atom_name: str, config: ShapeConfig, default: BondTypeShapeConfig, scale: float):
        ...
        self.chamfer_length = config.chamfer_length if config.chamfer_length is not None else (default.chamfer_length or 0.1)

        # shaft_length: mm単位が指定されていればそちらを優先
        if config.shaft_length_mm is not None:
            self.shaft_length = config.shaft_length_mm / scale
        elif default.shaft_length_mm is not None:
            self.shaft_length = default.shaft_length_mm / scale
        else:
            self.shaft_length = config.shaft_length if config.shaft_length is not None else (default.shaft_length or 0.3)

        # shaft_radius: mm単位が指定されていればそちらを優先
        if config.shaft_radius_mm is not None:
            self.shaft_radius = config.shaft_radius_mm / scale
        elif default.shaft_radius_mm is not None:
            self.shaft_radius = default.shaft_radius_mm / scale
        else:
            self.shaft_radius = config.shaft_radius if config.shaft_radius is not None else (default.shaft_radius or 0.3)

        # hole_length: mm単位が指定されていればそちらを優先
        if config.hole_length_mm is not None:
            self.hole_length = config.hole_length_mm / scale
        elif default.hole_length_mm is not None:
            self.hole_length = default.hole_length_mm / scale
        else:
            self.hole_length = config.hole_length if config.hole_length is not None else (default.hole_length or 0.3)

        # hole_radius: mm単位が指定されていればそちらを優先
        if config.hole_radius_mm is not None:
            self.hole_radius = config.hole_radius_mm / scale
        ...
        self.taper_radius_scale = config.taper_radius_scale if config.taper_radius_scale is not None else (default.taper_radius_scale or 1.0)
        self.taper_angle_deg = config.taper_angle_deg if config.taper_angle_deg is not None else (default.taper_angle_deg or 0.0)
